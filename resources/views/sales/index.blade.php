                                ...
                                <div class="flex space-x-2">
                                    @if(in_array(auth()->user()->role->name, ['SuperAdministrador', 'Administrador']))
                                        ...
                                        <form action="{{ route('sales.destroy', $sale) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg text-xs transition-colors duration-200 flex items-center" onclick="return confirm('¿Estás seguro de eliminar esta venta? El stock será devuelto al producto.')">
                                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
